...
public class Superstructure extends SubsystemBase {
    ...
    // Ajustée en fonction de la vitesse du robot
    ...
    // trouve entre deux distances. C'est déjà implémenté pour les doubles.
    InterpolatingTreeMap<Double, ShotParams> lutShotParams =
    ...
    public Superstructure(
        ...
        // Création de la LUT. Il faut ajouter la librairie OpenCSV dans le build.gradle
        try {
    ...
    // C'est l'angle par rapport à l'origine de la tourelle.
    public double getAngleCible(Translation2d cible) {
        ...
        // Tourelle pointe par en arrière
    ...
    // Si on ne spécifie pas de checkTourelle, on veut le centre du robot !
    public boolean isProche(Pose2d cible, double rayon) {
    ...
    //Calcul de l'activation du hub pour Dashboard et vibration
    ...
    public boolean hasWinAuto() {
        if (DriverStation.isEnabled()) {
            ...
            return switch(gameData.charAt(0)) {
                case 'R' -> Constants.isRedAlliance();
                case 'B' -> !Constants.isRedAlliance();
                default -> false;
            };
        }
        else {
